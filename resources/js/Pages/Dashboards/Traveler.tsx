        ...
        <aside className="hidden border-r border-slate-200 bg-white px-4 py-6 lg:block">
            <div className="sticky top-6 flex h-[calc(100vh-3rem)] flex-col">
                ...
                <div className="mt-8 px-3">
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Traveler</p>
                </div>
                ...
                <div className="mt-auto rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    <h3 className="text-sm font-semibold text-slate-950">Become a driver</h3>
                    <p className="mt-2 text-sm leading-6 text-slate-500">Publish rides and earn from empty seats.</p>
                    <Link href={path('drivers.onboarding.create')} className="mt-4 inline-flex w-full items-center justify-center rounded-lg bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white">Get started</Link>
                </div>
            </div>
        </aside>
    );
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm leading-6 text-slate-500">No upcoming trips are scheduled yet.</p>
                    </div>
                    <Link href={path('rides.search')} className="inline-flex shrink-0 items-center justify-center rounded-lg bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white">Find a ride</Link>
                </div>
            </section>
        );
    }
            ...
            <div className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-5">
                <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                    <div className="flex min-w-0 gap-4">
                        ...
                        <div className="min-w-0">
                            <h3 className="break-words text-xl font-semibold leading-tight tracking-tight text-slate-950">{ride?.departure_city?.name ?? 'Departure'} <span className="text-slate-300">→</span> {ride?.arrival_city?.name ?? 'Arrival'}</h3>
                            <div className="mt-4 flex flex-wrap gap-3 text-sm font-medium text-slate-600">
                                <span className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2"><IconCalendar className="h-4 w-4 text-brand-700" /> {ride?.departure_date ?? 'Date not set'}</span>
                                <span className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2"><IconClock /> {ride?.departure_time_label ?? ride?.departure_datetime_label ?? 'Time not set'}</span>
                                <span className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2"><IconPin /> {ride?.meeting_point ?? 'Meeting point not set'}</span>
                            </div>
                        </div>
                    </div>
                </div>
...
function TripInfo({ label, value, sub }: { label: string; value: string; sub?: string }) {
    return (
        <div className="min-w-0 rounded-lg border border-slate-200 bg-white px-3 py-3">
            ...
            {sub && <p className="mt-1 text-xs font-medium text-brand-700">{sub}</p>}
        </div>
    );
}
...
function BookingMobileCard({ booking }: { booking: Booking }) {
    ...
    return (
        <article className="rounded-xl border border-slate-200 bg-white p-4">
            ...
            <div className="mt-4 flex items-center justify-between gap-4 rounded-lg border border-slate-200 bg-white px-3 py-3 text-sm">
